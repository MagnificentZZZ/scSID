...
def load_fold_data(fold_idx):
    ...
    return train_tokens, train_labels, eval_tokens, eval_labels
...
def train_and_eval():
    ...
    for fold in range(1, FOLD + 1):
        print(f"\n=== Fold {fold}/{FOLD} ===")
        # 读取数据
        train_data, train_labels, eval_data, eval_labels = load_fold_data(fold)
        ...
        train_dataset = FoldDataset(train_data, train_labels)
        eval_dataset = FoldDataset(eval_data, eval_labels)
        ...
        for epoch in tqdm(range(EPOCH), desc=f'Fold {fold}/{FOLD}'):
            ...
            with torch.no_grad():
                all_val_predictions = []
                all_val_labels = []
                for val_data_batch, val_label_batch in eval_loader:
                ...
                val_ari = adjusted_rand_score(all_val_labels, val_pred_classes)
                ...
                if epoch == EPOCH - 1:
                    ...
                    last_nmis.append(val_nmi)
                    last_accs.append(val_accuracy)
                    last_precisions.append(val_precision_final)
                    # 保存模型
                    save_path = os.path.join(SAVE_DIR, f"{DATASET_NAME}_fold{fold}_final.pth")
                    torch.save({
                    ...
                    print(f"Saved model for fold {fold} at {save_path}")
    ...
    print(f"\nFinal Epoch F1 across {FOLD} folds: {mean_std_str(last_f1s)}")
    print(f"Final Epoch ARI across {FOLD} folds: {mean_std_str(last_aris)}")
    print(f"Final Epoch NMI across {FOLD} folds: {mean_std_str(last_nmis)}")
    print(f"Final Epoch Accuracy across {FOLD} folds: {mean_std_str(last_accs)}")
    print(f"Final Epoch Precision across {FOLD} folds: {mean_std_str(last_precisions)}")
